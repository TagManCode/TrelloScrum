...
var SETTING_NAME_DIFFS= "diffSequence";
var S4T_ALL_SETTINGS = [SETTING_NAME_LINK_STYLE, SETTING_NAME_ESTIMATES, SETTING_NAME_DIFFS];
...
S4T_SETTING_DEFAULTS[SETTING_NAME_DIFFS] = _diffSeq.join();
...
function showSettings()
{
	...
	// Create the Settings form.
	{
		...
		var setting_diffSeq = S4T_SETTINGS[SETTING_NAME_DIFFS];
		...
		// Which difference buttons should show up.
		var fieldset_diffButtons = $('<fieldset/>', {style: 'margin-top:5px'});
		var legend_diffButtons = $('<legend/>');
		legend_diffButtons.text("Difference Buttons");
		fieldset_diffButtons.append(legend_diffButtons);
			var diffExplanation = $('<div/>').text("List out the values you want to appear on the difference buttons, separated by commas. They can be positive or negative whole numbers, or decimals.");
			fieldset_diffButtons.append(diffExplanation);
			
			var diffFieldId = 'diffPointSequenceToUse';
			var diffField = $('<input/>', {id: diffFieldId, size: 40, val: setting_diffSeq});
			fieldset_diffButtons.append(diffField);
			
			var diffTitleTextStr = "Original sequence is: " + _diffSeq.join();
			var diffRestoreDefaultsButton = $('<button/>')
											.text('restore to original values')
											.attr('title', diffTitleTextStr)
											.click(function(e){
												e.preventDefault();
												$('#'+settingsFrameId).contents().find('#'+diffFieldId).val(_diffSeq.join());
											});
			fieldset_diffButtons.append(diffRestoreDefaultsButton);
		...
		var saveButton = $('<button/>', {style:'margin-top:5px'}).text('Save Settings').click(function(e){
			...
			S4T_SETTINGS[SETTING_NAME_DIFFS] = $('iframe').contents().find('#'+diffFieldId).val();
		...
		settingsForm.append(fieldset_diffButtons);
	...
	var quickStartDiv = $('<div>\
		...
		<ol style="margin-top:0px">\
			...
			<li>If the story consumes more or less story points than what was estimated, use the <strong>difference</strong> buttons to adjust.<br/>\
			For example, if a story is estimated to consume 8 points, but actually takes 10, <strong>estimated</strong> remains at 8,\
			and <em>difference</em> is set as +2.</li>\
		</ol>\
	</div>');

	var moreInfoLink = $('<small>For more information, see <a href="//scrumfortrello.com">ScrumForTrello.com</a></small>');
...
function computeTotal(){
	...
	ctto = setTimeout(function(){
		...
		for (var i in _pointsAttr){
			...
			var scoreSpan = $('<span/>', {class: attr}).text(sign + (round(score)||''));
	...
	this._calcInner	= function(e){ // don't call this directly. Call calc() instead.
		...
		to = setTimeout(function(){
			...
			for (var i in _pointsAttr){
				...
				// Sign to display. Only show a '+' if this
				// if cpoints and it's positive.
				var sign = '';
				if (attr === 'cpoints' && scoreTruncated !== 0) {
					sign = (scoreTruncated > 0) ? '+' : '';
				}
				sign += scoreTruncated;

				var scoreSpan = $('<span/>', {class: attr}).text(sign);
				$total.append(scoreSpan);
				computeTotal();
			}
		});
...
function showPointPicker(location) {
	...
	$picker.append($('<span>').text('Estimated: '));
	...
	var diffSequence = (S4T_SETTINGS[SETTING_NAME_DIFFS]).split(',');
	$picker.append($('<span>').text('Difference: '));
	for (var i in diffSequence) $picker.append($('<span>', {class: "point-value"}).text(diffSequence[i]).click(function(){
		var value = $(this).text();
		var $text = $('.card-detail-title .edit textarea');
		var text = $text.val();
